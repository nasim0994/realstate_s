...
import { useAddAwardsMutation, useGetAwardsByIdQuery, useUpdateAwardsMutation } from '@/redux/features/awards/awardsApi';
...
export default function AwardsForm() {
    ...
    const { data: awardData, isLoading: isFetching } = useGetAwardsByIdQuery(id, { skip: !id });
    const [addAwards, { isLoading: isCreating }] = useAddAwardsMutation();
    const [updateAwards, { isLoading: isUpdating }] = useUpdateAwardsMutation();

    const { register, handleSubmit, watch, setValue, reset, control, formState: { errors } } = useForm();

    useEffect(() => {
        if (isEditMode && awardData?.data) {
            reset(awardData?.data);
        }
    }, [awardData, reset, isEditMode]);
    ...
    const onSubmit = async (data: any) => {
        ...
        formData.append('data', JSON.stringify(awardInfo));
        ...
        try {
            const res = isEditMode
                ? await updateAwards({ id, data: formData }) as TResponse
                : await addAwards(formData) as TResponse;

            if (res.data?.success) {
                toast.success(isEditMode ? "Award updated successfully!" : "Award created successfully!");
                navigate('/admin/awards/all');
            } else {
                toast.error(
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
            <div className="flex items-center justify-between bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm">
                <div className="flex items-center gap-4">
                    <Link to="/admin/awards/all" className="p-2 hover:bg-slate-100 rounded-xl transition-all">
                        <ArrowLeft size={20} />
                    </Link>
                    <h1 className="text-xl font-bold text-neutral">{isEditMode ? 'Edit Award' : 'Add Award'}</h1>
                </div>
                <button type="submit" disabled={isCreating || isUpdating} className="admin_primary_btn min-w-35">
                    <Save size={18} /> {isEditMode ? 'Update Award' : 'Publish Award'}
                </button>
            </div>
                ...
                <div className="lg:col-span-2 space-y-4">
                    <div className="bg-white p-8 rounded-3xl border border-slate-200/60 space-y-5">
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><Type size={14} /> Award Title</label>
                            <input type="text" {...register("title", { required: true })} placeholder="Enter catchy title..." />
                        </div>

                        <div className='grid sm:grid-cols-2 md:grid-cols-3 gap-4'>
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><Type size={14} />Organization</label>
                                <input type="text" {...register("organization", { required: true })} />
                            </div>

                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><Type size={14} />Year</label>
                                <input type="number" {...register("year", { required: true })} />
                            </div>

                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-2"><Type size={14} />Order</label>
